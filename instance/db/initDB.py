import createDB
from src.app.shop import cur_path, src, parent
import os
...
with createDB.app.app_context():
    for category in lookup_data:
        for category_name in lookup_data[category]:
            newCategory = createDB.ItemClassification(classification_id=category_name["classification_id"],
                                                      id_identifier=category_name["id_identifier"],
                                                      category_name=category_name["category_name"])
            createDB.db.session.add(newCategory)

        for country in lookup_data["Country list"]:
            newCountry = createDB.CountryList(country_id=country["country_id"],
                                              country_name=country["country_name"])
            createDB.db.session.add(newCountry)

        for vendor in lookup_data["Card classification"]:
            newVendor = createDB.CardClassification(classification_id=vendor["classification_id"],
                                                    id_identifier=vendor["id_identifier"],
                                                    card_type=vendor["card_type"])
            createDB.db.session.add(newVendor)

    for category in item_data:
        for item in item_data[category]:
            newListing = createDB.ItemListing(listing_id=item["listing_id"],
                                              id_identifier=item["listing_id"][0],
                                              listing_name=item["listing_name"],
                                              thumbnail_img=item["thumbnail_img"],
                                              listing_description=item["listing_description"],
                                              avg_price=item["avg_price"], avg_rating=item["avg_rating"],
            ...
            for index in range(len(creating_table_for["option_id"])):
                newOption = createDB.ListingOptions(option_id=creating_table_for["option_id"][index],
                                                    for_listing_id=item["listing_id"],
                                                    ...
                                                    options_stock=creating_table_for["options_stock"][index],
            ...
            for index in range(len(creating_table_for["billing_id"])):
                newBilling = createDB.ListingBillingInfo(listing_id=item["listing_id"],
                                                         billing_id=creating_table_for["billing_id"][index],
                                                         address_1=creating_table_for["address_1"][index],
                                                         ...
                                                         phone_area_code=creating_table_for["phone_area_code"][index],
                                                         phone_num=creating_table_for["phone_number"][index],
                                                         payment_method=creating_table_for["payment_method"][index],
                                                         billing_type="listing")

            creating_table_for = item["payment_method"]
            for index in range(len(creating_table_for["payment_id"])):
                newPaymentMethod = createDB.CreditCardDetails(details_id=creating_table_for["payment_id"][index],
                                                              payment_type=creating_table_for["transaction_type"][index],
                                                              card_identifier=creating_table_for["card_identifier"][index],
                                                              card_num=creating_table_for["card_num"][index],
                                                              expiry_date=creating_table_for["expiry_date"][index],
                                                              cardholder=creating_table_for["cardholder"][index])
                newBilling.details.append(newPaymentMethod)
                newListing.billing.append(newBilling)
